...
function buildPlaneMesh() {
  ...
  boxBuilder(pos, nrm, col, idx, 0, 0, 0,        1.10, 0.95, 2.40, BODY);
  boxBuilder(pos, nrm, col, idx, 0, 0, -1.55,    0.75, 0.65, 0.60, BODY);
  boxBuilder(pos, nrm, col, idx, 0, 0.55, -0.20, 0.72, 0.30, 0.95, GLASS);
  boxBuilder(pos, nrm, col, idx, 0, 0.05, 0,     6.60, 0.18, 1.20, WING);
  boxBuilder(pos, nrm, col, idx, -3.00, 0.05, 0, 0.60, 0.28, 0.95, ACCENT);
  boxBuilder(pos, nrm, col, idx,  3.00, 0.05, 0, 0.60, 0.28, 0.95, ACCENT);
  boxBuilder(pos, nrm, col, idx, 0, 0.70, 1.05,  0.14, 0.95, 0.75, BODY);
  boxBuilder(pos, nrm, col, idx, 0, 0.20, 1.10,  2.30, 0.14, 0.65, WING);

  return createMesh(new Float32Array(pos), new Float32Array(nrm),
                    new Float32Array(col), new Uint16Array(idx));
}
...
function buildPropMesh() {
  ...
  boxBuilder(pos, nrm, col, idx, 0, 0, 0,    3.4, 0.28, 0.07, BLADE);
  boxBuilder(pos, nrm, col, idx, 0, 0, 0,    0.28, 3.4, 0.07, BLADE);
  boxBuilder(pos, nrm, col, idx, 0, 0, 0.10, 0.42, 0.42, 0.34, HUB);
...
function buildTreeMesh() {
  ...
  /* Simple low-poly fallback tree. It is intentionally box-based so it
     matches the existing game's low-poly visual language. */
  boxBuilder(pos, nrm, col, idx, 0, 1.6, 0, 0.55, 3.2, 0.55, TRUNK);
  boxBuilder(pos, nrm, col, idx, 0, 3.2, 0, 3.8, 2.4, 3.8, LEAF1);
  boxBuilder(pos, nrm, col, idx, 0, 4.9, 0, 3.0, 2.1, 3.0, LEAF2);
  boxBuilder(pos, nrm, col, idx, 0, 6.35, 0, 2.0, 1.7, 2.0, LEAF3);
...
function loadAssetTexture(name, fallback) {
  ...
  assets[name] = { texture: tex, loaded: false, path: assetPaths[name] };
...
function init(glContext) {
  gl = glContext;
  models.planeMesh = buildPlaneMesh();
  models.propMesh = buildPropMesh();
  models.treeMesh = buildTreeMesh();
  loadAssets();
}
...
function buildChunk(cx, cz) {
  ...
  /* Deterministic trees: the same chunk always gets the same layout. */
  const objects = [];
  const seed = (cx * 73856093) ^ (cz * 19349663);
  const count = 2 + ((Math.abs(seed) >>> 0) % 3);
